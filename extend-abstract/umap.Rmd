...
## Introduction
UMAP is a nonlinear dimensionality reduction technique with strong mathematical footing. Published in 2018 [@2018arXivUMAP], UMAP is a new graph layout algorithm (such as Local Linear Embedding [@Roweis2000] and t-SNE [@2008tsne]) built on the ideas of Riemannian geometry and Algebraic topology.

## Theoretical foundation
UMAP is a two-fold algorithm. In a first step, a high-dimensional weighted neighborhood-graph representation of the data is built.
The second step requires optimizing a low-dimensional (in euclidean space) representation of data to minimize the cross-entropy between the two topological representations.
...
[@2018arXivUMAP] provides comparisons of UMAP with other algorithms, notably t-SNE and LargeVis [@TangLZM16].
In terms of quality of the embedding, UMAP performs similarly to both t-SNE and LargeVis. UMAP however tends to preserve more of the global structure of the data.
Computationally, the run time of UMAP is better than of t-SNE and LargeVis, and it scales better to embeddings in dimensions higher than two and larger datasets.
